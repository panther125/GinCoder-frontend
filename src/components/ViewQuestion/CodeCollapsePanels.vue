    ...
    <a-card class="controlCard">
      ...
      >
        <a-tabs size="mini" v-model:activeKey="runContent.activeKey">
          ...
          <a-tab-pane key="2" title="执行结果" style="font-size: 18px;">
            <a-scrollbar
              ...
              >
                <div v-if="resultData">
                  ...
                  <span class="info">
                    消耗内存：
                    <p class="info" v-if="resultData.time">
                      {{ resultData.memory }} MB
                    </p>
                    <p class="info" v-else>N/A</p>
                  </span>
                  <div class="message">
                    <div class="messageBox">测试输出： {{ resultData.message }}</div>
                  </div>
                </div>
...
import { SIZES } from "@arco-design/web-vue/es/_utils/constant";
